...
class NoteController {
    ...
    public function uploadImage(int $id): void {
        ...
        if (move_uploaded_file($file['tmp_name'], $savePath)) {
            $imageId = $this->noteModel->addImage($id, $urlPath); // ← lưu id
            echo json_encode([
                'status' => 'success',
                'id'     => $imageId,  // ← thêm id vào response
                'url'    => $urlPath
            ]);
        } else {
    ...
    public function deleteImage(int $id): void {
        ...
        // Lấy path ảnh trước khi xóa để unlink file vật lý
        $stmt = $this->noteModel->getImageById($image_id, $id);
        if ($stmt) {
            $relativePath = str_replace(BASE_URL, '', $stmt['path']);
        ...
        if ($this->noteModel->deleteImage($image_id, $id)) {
            echo json_encode(['status' => 'success', 'message' => 'Đã xóa ảnh thành công']);
        } else {
            echo json_encode(['status' => 'error', 'message' => 'Không thể xóa ảnh trong CSDL']);
        }
    }
}