...
// ✅ FIX: Đọc note_color từ session preferences
$noteColor = $_SESSION['preferences']['note_color'] ?? '#ffffff';
?>
...
function renderNoteCard(array $note, string $noteColor = '#ffffff'): string {
    $base    = BASE_URL; // ✅
    $id      = $note['id'];
    ...
    <div class='col note-card-wrapper' data-note-id='$id'>
        <div class='card h-100 shadow-sm border-0 note-card'
             onclick=\"$onclick\" style='cursor:pointer; background-color:{$noteColor} !important;'>
            <div class='card-body p-3'>
