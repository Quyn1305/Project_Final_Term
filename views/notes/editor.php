...
$fontSizeMap = ['small' => '13px', 'medium' => '15px', 'large' => '18px'];
...
<div class="container mt-3 fade-in">
    <div class="row justify-content-center">
        <div class="col-lg-8 col-xl-7">
            ...
            <!-- NOTE CARD -->
            <div class="card shadow-sm border-0 rounded-4" style="background-color: <?= htmlspecialchars($noteColor) ?>;">
                <div class="card-body p-4">
                    ...
                    <input type="text"
                        id="note_title"
                        class="form-control border-0 fw-bold mb-3 px-0"
                    ...
                    <textarea id="note_content"
                            ...
                            style="resize:none; font-size: <?= $fontSizePx ?>;"
                        ...
                        <div id="images-container" class="d-flex flex-wrap gap-2">
                            <?php if ($isEdit && !empty($noteImages)): ?>
                                ...
                                <div class="position-relative image-item"
                                     data-id="<?= $img['id'] ?>"
                                     data-url="<?= htmlspecialchars($img['path']) ?>">
                                    ...
                                    <button class="btn btn-danger btn-sm position-absolute top-0 end-0 p-0"
                                            ...
                                            onclick="deleteImage(<?= $img['id'] ?>, '<?= htmlspecialchars($img['path']) ?>')">
                                        ×
                                    </button>
                                </div>
                                <?php endforeach; ?>
...
<!-- [NEW] Modal xem ảnh full size khi click vào thumbnail -->
<div class="modal fade" id="imageLightboxModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content border-0 shadow-lg"
             style="background:rgba(0,0,0,0.85);">
            <div class="modal-header border-0 pb-1">
                <span id="lightbox-filename" class="text-white small opacity-75"></span>
                <div class="d-flex gap-2 ms-auto">
                    <!-- Nút tải ảnh về -->
                    <a id="lightbox-download"
                       href="#"
                       download
...
<script>
// [FIX] Khai báo NOTE_ID và hasPassword ở ngoài cùng, TRƯỚC tất cả các hàm
// để deleteImage() và các hàm khác luôn dùng được biến này dù thứ tự nào.
...
// [FIX + NEW] addImagePreview: thêm onclick mở lightbox vào ảnh mới upload
function addImagePreview(imgId, url) {
    ...
    div.innerHTML   = `
        ...
        <button class="btn btn-danger btn-sm position-absolute top-0 end-0 p-0"
                style="width:18px;height:18px;font-size:10px;line-height:1;"
                onclick="deleteImage(${imgId}, '${url}')">×</button>`;
    container.appendChild(div);
}

// [NEW] Mở lightbox xem ảnh full size
function openLightbox(url) {
    ...
    // Hiện tên file từ URL (phần sau dấu / cuối cùng)
    const name = url.split('/').pop().split('?')[0];
    filename.textContent = decodeURIComponent(name);
...
// [FIX] deleteImage nhận cả imgId lẫn imageUrl
// Backend dùng image_url để xóa file vật lý + DB, không dùng image_id
function deleteImage(imgId, imageUrl) {
    if (!NOTE_ID) { alert('Không tìm thấy ghi chú!'); return; }

    const lightboxEl = document.getElementById('imageLightboxModal');
    const lightbox   = bootstrap.Modal.getInstance(lightboxEl);
    ...
    fetch(`${BASE_URL}/notes/${NOTE_ID}/delete-image`, {
        method  : 'POST',
        headers : { 'Content-Type': 'application/x-www-form-urlencoded' },
        body    : `image_id=${imgId}`   // ← gửi image_id, khớp với model
    })
    .then(res => res.json())
    .then(data => {
        if (data.status === 'success') {
            const item = document.querySelector(`.image-item[data-id="${imgId}"]`);
            if (item) {
            ...
            showToast(data.message || 'Không thể xóa ảnh!', 'error');
        }
    })
