...
// ===== Highlight + áp dụng NGAY khi click theme =====
document.querySelectorAll('.theme-radio').forEach(radio => {
    radio.addEventListener('change', function() {
        ...
        this.closest('label').querySelector('.theme-option')
            .classList.add('border-primary', 'border-2', 'bg-primary', 'bg-opacity-10');
        
        // Áp dụng theme ngay lập tức khi click
        document.documentElement.setAttribute('data-theme', this.value);
    });
});

// ===== Highlight + áp dụng NGAY khi click font size =====
document.querySelectorAll('.fontsize-radio').forEach(radio => {
    radio.addEventListener('change', function() {
        document.querySelectorAll('.font-option').forEach(el => {
            el.classList.remove('border-primary', 'border-2', 'bg-primary', 'bg-opacity-10');
        });
        this.closest('label').querySelector('.font-option')
            .classList.add('border-primary', 'border-2', 'bg-primary', 'bg-opacity-10');

        // Áp dụng font size ngay lập tức
        const sizeMap = { small: '13px', medium: '15px', large: '18px' };
        document.body.style.fontSize = sizeMap[this.value] || '15px';
    });
});
// ===== Highlight màu nền khi click =====
