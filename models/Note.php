...
class Note {
    ...
    // ================= IMAGES (Tiêu chí 15) =================
    public function addImage(int $note_id, string $path): int {
        $stmt = $this->conn->prepare("
            INSERT INTO note_images (note_id, path) VALUES (?, ?)
        ");
        $stmt->execute([$note_id, $path]);
    ...
    public function getImageById(int $image_id, int $note_id): ?array {
        $stmt = $this->conn->prepare("
            SELECT * FROM note_images 
            WHERE id = ? AND note_id = ?
        ");
        $stmt->execute([$image_id, $note_id]);
